...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  const toggleSidebar = React.useCallback(() => {
    if (isMobile) { setOpenMobile(o => !o) } else { setOpen(!open) }
  ...
  if (isMobile) {
    return (
      <div
        data-mobile="true"
        className={cn(
          "fixed inset-y-0 z-50 flex h-full flex-col bg-sidebar text-sidebar-foreground transition-transform duration-300",
      ...
      >
        {openMobile && (
          <div className="fixed inset-0 bg-black/50 -z-10" onClick={() => setOpenMobile(false)} />
        )}
        {children}
      </div>
    )
